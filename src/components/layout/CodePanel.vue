<template>
  ...
  >
    <div class="absolute bottom-full flex w-full items-center border-t-2 rounded-t-xl bg-white">
      <div class="flex-1 px-3">
        <svg class="h-[32px] w-[32px] cursor-pointer" @click="shownHelp = true">
          <path
    ...
    <div class="p-4">
      <button
        ...
        :disabled="!active"
        :class="[
          active ? 'bg-black text-white' : 'opacity-30 bg-white',
          wrong ? 'bg-red' : null,
          'w-full rounded-xl py-2 text-xl font-medium',
        ]"
        @click="verifyCode"
      >
  ...
  <HelpModal :visible="shownHelp" @close="shownHelp = false"></HelpModal>
</template>
