...
export function useSurvey() {
  ...
  const SURVEY_ID =
    getActiveLanguage() === 'sk' ? import.meta.env.VITE_SURVEY_SK : import.meta.env.VITE_SURVEY_EN

  const shouldLaunch = computed(
    () => (viewedItemsCount.value > 1 || stories.value.length > 4) && !wasExited.value
  )
  function done() {
    isDone.value = true
  }
